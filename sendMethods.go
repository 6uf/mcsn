		...
		for i, accountType := range payload.AccountType {
			...
			for f := 0; f < leng; {
				if statuscodes[num] != "200" {
					...
					bearerGot, emailGot, got := check(statuscodes[num], name, fmt.Sprintf("%v", dropTime))

					content += fmt.Sprintf("+ [%v] Succesfully sniped %v | %v\n", statuscodes[num], name, emailGot)
					fmt.Print(aurora.Sprintf(aurora.Bold(aurora.White(("[%v] Rec @ %v | %v\n"))), aurora.Bold(aurora.Green(statuscodes[num])), aurora.Bold(aurora.Green(formatTime(recvs[num]))), aurora.Bold(aurora.Green(strings.Split(emailGot, ":")[0]))))
