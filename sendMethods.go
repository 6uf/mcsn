		...
		for i, accountType := range payload.AccountType {
			...
			for f := 0; f < leng; {
				if statuscodes[f] != "200" {
					...
					if got {
						log.Println(aurora.Sprintf(aurora.Bold(aurora.White("[%v] Sent Webhook")), aurora.Bold(aurora.Green("204"))))
					} else {
						log.Println(aurora.Bold(aurora.White("[FAILED] Couldnt send webhook.")))
					}
					...
					if err != nil {
						...
						if req.StatusCode == 200 {
							log.Println(aurora.Sprintf(aurora.Bold(aurora.White("[%v] Changed Skin")), aurora.Bold(aurora.Green("200"))))
						} else if req.StatusCode == 400 {
							log.Println(aurora.Sprintf(aurora.Bold(aurora.White("[%v] Used wrong bearer during Skin Change. (acc is giftcard :skull:)")), aurora.Bold(aurora.Red("400"))))
						} else {
							log.Println(aurora.Sprintf(aurora.Bold(aurora.White("[%v] Unauthorized")), aurora.Bold(aurora.Green("401"))))
						}
					}
