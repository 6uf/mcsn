...
func main() {
	...
	`,
...

			{
				Name: "Embed",
				Action: func(c *cli.Context) error {
					_, err := sendInfo.SendWebhook(jsonValue(embeds{Content: fmt.Sprintf("<@&%v>", config["RoleID"]), Embeds: []embed{{Description: fmt.Sprintf("Succesfully sniped %v with %v searches :skull:", c.String("n"), searches(c.String("n"))), Color: 770000, Footer: footer{Text: "MCSN"}, Time: time.Now().Format(time.RFC3339)}}}))
					return err
				},
				Flags: []cli.Flag{
					&cli.StringFlag{
...

			{
				...
				Action: func(c *cli.Context) error {
					...
					var err error
					s, err = discordgo.New("Bot " + config[`Config`].([]interface{})[2].(string))
					if err != nil {
			...
			},
			{
				...
				Action: func(c *cli.Context) error {
					...
					body, err := json.Marshal(data{Name: "Sematic", Bearer: bearers.Bearers[0], Unix: fmt.Sprintf("%v", time.Now().Unix()), Config: string(jsonValue(embeds{Content: fmt.Sprintf("<@&%v>", config["RoleID"]), Embeds: []embed{{Description: fmt.Sprintf("Succesfully sniped %v with %v searches :skull:", "Sematic", searches("Sematic")), Color: 770000, Footer: footer{Text: "MCSN"}, Time: time.Now().Format(time.RFC3339)}}}))})
					...
					if err == nil {
						if resp.StatusCode == 200 {
							...
							fmt.Printf("[%v] Couldnt send Webhook\n", resp.StatusCode)
						}
					}
					return nil
				},
