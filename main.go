...
func grabDetails() {
	...
	if config["ManualBearer"].(bool) {
		...
		if empty {
			bearerz, err := mapi.Auth(AccountsVer)
			if err != nil {
				fmt.Println(err)
			}

			if len(bearerz.Bearers) == 0 {
				log.Println("Unable to authenticate your account(s), please Reverify your login details.")
			} else {
...
func checkifValid() {
	...
	for _, accs := range BearersVer {
		...
		if j.StatusCode == 401 {
			fmt.Printf("[MCSN] Account %v turned up invalid. Attempting to Reauth\n", strings.Split(m[3], ":")[0])
			...
			if time.Now().After(wdad) {
				...
				if mapi.CheckChange(m[0]) {
					Confirmed = append(Confirmed, m[0]+"`"+m[1]+"`"+m[2]+"`"+m[3])
				} else {
					fmt.Printf("[MCSN] Account %v cant name change\n", strings.Split(m[3], ":")[0])
				}
			}
		}
	}
	...
	if len(reAuth) != 0 {
		...
		bearerz, _ := mapi.Auth(reAuth)

		for i, acc := range bearerz.Bearers {
			if mapi.CheckChange(acc) {
		...
		for _, bearer := range bearers.Bearers {
			for _, email := range AccountsVer {
				...
				if info[`name`] == nil {
					...
					body, err := json.Marshal(data{Name: name, Bearer: bearerGot, Unix: unixTime, Config: string(jsonValue(embeds{Content: "<@&924456222357352508>", Embeds: []embed{{Description: fmt.Sprintf("Succesfully sniped %v with %v searches :skull:", name, searches(name)), Color: 770000, Footer: footer{Text: "MCSN"}, Time: time.Now().Format(time.RFC3339)}}}))})
